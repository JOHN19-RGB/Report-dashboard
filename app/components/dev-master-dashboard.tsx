...
const DEFAULT_FILTERS: DevReportFilters = { search: "", startDate: "2026-01-01", endDate: "2026-12-31", taskType: "all" };
...
export default function DevMasterDashboard() {
  ...
  async function loadData(refresh: boolean) {
    ...
    try {
      ...
      setFilters(current => ({
        ...current,
        startDate: `${payload.reportYear}-01-01`,
        endDate: `${payload.reportYear}-12-31`,
  ...
  const hasFilters = JSON.stringify(filters) !== JSON.stringify({ ...DEFAULT_FILTERS, startDate: `${data?.reportYear || 2026}-01-01`, endDate: `${data?.reportYear || 2026}-12-31` });
  let donutPosition = 0;
  ...
  function resetFilters() {
    ...
    setFilters({ ...DEFAULT_FILTERS, startDate: `${year}-01-01`, endDate: `${year}-12-31` });
  }
      ...
      <div className="content-wrap dev-dashboard-wrap">
        <section className="dev-dashboard-heading">
          ...
          <div className="dev-dashboard-filters" aria-label="Dev тайлангийн шүүлтүүр">
            ...
            <label className="dev-type-filter"><ListFilter size={17} /><span>Task Type:</span><select value={filters.taskType} onChange={event => updateFilter("taskType", event.target.value)}><option value="all">All Types</option>{taskTypes.map(type => <option key={type} value={type}>{type}</option>)}</select><ChevronDown size={14} /></label>
        ...
        <section className="dev-chart-grid">
          <article className="dev-panel dev-performance-panel">
            <header><div><h2>Task Performance Comparison</h2><p><span className="bug" /> Bug <span className="imp" /> Imp</p></div><span className="dev-period-badge"><CalendarDays size={14} /> {data?.reportYear || 2026}</span></header>
